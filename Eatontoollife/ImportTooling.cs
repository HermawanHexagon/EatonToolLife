...
{
    public partial class ImportTooling : Form
    {
        Sqloperation Mydb = new Sqloperation();
        public string Path;

        public ImportTooling()
        {
            InitializeComponent();
        }
        ...
        {
            public string[] Collectdata = new string[13];
        }
        ...
        private void ImportTooling_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (Bgworker.IsBusy)
            {
            ...
            if (DGImport.Rows.Count > 0)
            {
                try
                {
                    ...
                    if (result == System.Windows.Forms.DialogResult.Yes)
                    {
                        ...
                        {
                            Mydb.Savedatabasetoolingnumberlist(DGImport.Rows[CounterRow].Cells[0].Value.ToString(), DGImport.Rows[CounterRow].Cells[1].Value.ToString(),
                            ...
                            DGImport.Rows[CounterRow].Cells[12].Value.ToString());
